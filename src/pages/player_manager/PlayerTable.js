...
const PlayerTable = () => {
  ...
  const Filter = (searchText) => {
    ...
    setRecords(
      ...
      player.category.toLowerCase().includes(searchText.toLowerCase().trim()) 
      )
    );
  };
  ...
  return (
    <>
      <div className="">
        ...
        <DatePickerPlayer onFilter={Filter} onFilterByDate= {FilterByDate} setRecords={setRecords} playerData={playerData} />
  ...
  <input type="checkbox" checked={player.status==="active" ? true : false} onClick={() =>handleShow(player)} />
  <span className="slider round"></span>
</label>
                  </td>
                  
                  <td className="border py-4 px-3 flex">
                  <RiEdit2Fill onClick={()=>modelview(player)} className="edit-icon" />
                  <MdDelete onClick={() =>deleteModelView(player)} className="delete-icon"/>
        ...
        <PlayerPagination totalItems={teamList.length}
      ...
      {modelShow && <UpdatePlayer show={modelShow} handleClose={modelview} dataView={fatchData} />}
      {showDeleteModel && <DeletePlayer showdel={showDeleteModel} deleteModelView={deleteModelView} dataView={fatchData}/>} 
         </>
  );
};
